...
int main() {
    ifstream inputStream("input/02_2.txt");
    ...
    while (getline(inputStream, inputLine)) {
        ...
        // Just do the brute force for the part two
        bool orSum = safe(levels);
        for (int i = 0; i < levels.size(); i++) {
            int removedLevel = levels[i];
            levels.erase(levels.begin() + i);

            orSum |= safe(levels);
            
            levels.insert(levels.begin() + i, removedLevel);
        }
        res += orSum;
    }

    // result
    ofstream output("output/02_2.txt");
